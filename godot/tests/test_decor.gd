...
## saving and loading with presets, the default arrangement, and the editor.
...
func test_default_decor_stays_clear_of_solid_zones() -> void:
	var main := await _main()
	main.editor.load_preset(LayoutPresets.DEFAULT)
	var cards: CardSystem = main.cards
	var decor: TankDecor = main.decor
	check(decor.pieces.size() >= 8, "the Default preset has decor (%d pieces)" % decor.pieces.size())
	for piece in decor.pieces:
		if not piece.is_solid():
			continue # plants are swim-through; they can't block a card
		for card in cards.cards:
			check(not piece.intersects(cards.zone_for(card.position)),
				"%s at %s stays out of %s's zone" % [piece.type, piece.position, card.binding.display_name()])
	main.queue_free()
