...
## cards (combos and sequences too), arrange decor, and save or load named
## presets holding both.
##
...
func _rebuild_card_panel() -> void:
	...
	if b.kind == CardBinding.Kind.HOLD:
		_label(_card_box, "Held while the fish stays. Pick one or more inputs:", 15, UiStyle.MUTED)
