...
func card_color() -> Color:
	...
	if binding.kind == CardBinding.Kind.HOLD and ids.size() == 1:
...
static func combined_arrow(b: CardBinding) -> Vector2:
	...
	if b.kind != CardBinding.Kind.HOLD or ids.size() < 2:
		return CardSystem.INPUTS[ids[0]].get("arrow", Vector2.ZERO) if ids.size() == 1 and b.kind == CardBinding.Kind.HOLD else Vector2.ZERO
	var group := ""
